...
def write_stories(schema_info, f_stories, f_Q_next, rand_seed, n_repeats):
    ...
    for i in range(n_repeats):
        ...
        event_scenes = write_one_story(schema_info, f_stories, f_Q_next)
...
def write_one_story(schema_info, f_stories, f_Q_next):
    ...
    while True:
        ...
        filled_Q, _, _ = get_filled_state(curr_state, grounding, states, attributes)
        ...
        filled, fillers, scene = get_filled_state(curr_state, grounding, states, attributes,
        ...
        if alt_future != 0:
            had_alt_future = True
            alt_future_filled, _, _ = get_filled_state(alt_future, grounding, states, attributes)
...
def get_filler_inconsistent_next_state(fillers, people_introduced, people_all,
    ...
    next_state_temp, next_fillers, _ = get_filled_state(curr_state, grounding, states, attributes)
    # print('->:%s\n' % next_state_temp)
...
def generate_alternative_grounding(next_characters, grounding, curr_state,
    ...
    for k in range(num_characters_next_state):
        ...
        if alt_next_grounding == grounding:
            ...
            alt_future_filled, _, _ = get_filled_state(curr_state, alt_next_grounding, states, attributes)
